...
namespace Dashboard.Services.Parsers
{
    ...
    public class VehicleHtmlParser : IHtmlParser
    {
        ...
        private IList<HtmlNode> SearchFor(IEnumerable<HtmlNode> nodes, string className)
        {
            foreach (var node in nodes)
            {
                yield return
                    nodes.Where(x => x.Attributes["class"] != null).Where(x => x.Attributes["class"].Value == className)
                    ;
            }
        }
