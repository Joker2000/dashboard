...
namespace Dashboard.Services.Parsers
{
    ...
    public class VehicleHtmlParser : IHtmlParser
    {
        ...
        private IEnumerable<HtmlNode> SearchFor(IEnumerable<HtmlNode> nodes, string className)
        {
            return
                nodes.Where(x => x.Attributes["class"] != null)
                    .Where(x => x.Attributes["class"].Value == className)
                    .ToList();
