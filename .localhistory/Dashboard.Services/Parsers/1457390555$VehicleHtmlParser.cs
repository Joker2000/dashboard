...
namespace Dashboard.Services.Parsers
{
    ...
    public class VehicleHtmlParser : IHtmlParser
    {
        ...
        /// </returns>
        private IList<HtmlNode> GetNodesWithAttribute(HtmlDocument document, string attribute)
        {
            var nodes = new List<HtmlNode>();
            nodes.AddRange(document.DocumentNode.Descendants().Where(x => x.Attributes[attribute] != null).ToList());

            return nodes;
        }

        private IList<HtmlNode> GetNodesWithAttribute(HtmlNode node, string attribute)
        {
            var nodes = new List<HtmlNode>();
            nodes.AddRange(node.ChildNodes.Where(x => x.Attributes[attribute] != null).ToList());
