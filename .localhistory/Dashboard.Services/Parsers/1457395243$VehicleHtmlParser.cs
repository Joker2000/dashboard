...
namespace Dashboard.Services.Parsers
{
    ...
    public class VehicleHtmlParser : IHtmlParser
    {
        ...
        {
            var vehicleDetails = SearchFor(document, "class", "ul-data");

            var vehicleDetailsList = vehicleDetails.ChildNodes.Where(x => x.Name == "li");

            var htmlNodes = new KeyValuePair<HtmlNode, HtmlNode>();

            foreach (var vehicleDetail in vehicleDetailsList)
            {
                htmlNodes.Add(vehicleDetail.ChildNodes.Single(x => x.Name == "span")
                    vehicleDetail.ChildNodes.Single(x => x.Name == "strong"));
            }
