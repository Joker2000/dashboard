...
namespace Dashboard.Services.Parsers
{
    ...
    public class VehicleHtmlParser : IHtmlParser
    {
        ...
        {
            var vehicleDetails = SearchFor(document, "Vehicle details");

            var regNumber = SearchFor(document, "class", "registrationNumber").Single().InnerText;
        ...
        /// </returns>
        private static IEnumerable<HtmlNode> SearchFor(HtmlDocument document, string attribute, string value)
        {
            return
                document.DocumentNode.Descendants()
                    .Where(x => x.Attributes[attribute] != null)
                    .Where(x => x.Attributes[attribute].Value == value)
                    .Last();
        }
        ...
        /// The <see cref="IEnumerable{HtmlNode}"/>.
        /// </returns>
        private static IEnumerable<HtmlNode> SearchFor(HtmlDocument document, string innerHtml)
        {
            return
                document.DocumentNode.Descendants()
                    .Where(x => x.InnerHtml.Contains(innerHtml))
                    .ToList();
        }
